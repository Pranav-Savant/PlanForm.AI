 ...
 */
const deepTrim = (value, maxItems = 3, maxStrLen = 120, maxDepth = 4, depth = 0) => {
  if (depth > maxDepth) return "…";

  if (Array.isArray(value)) {
    const sliced = value.slice(0, maxItems);
    const trimmed = sliced.map((v) => deepTrim(v, maxItems, maxStrLen, maxDepth, depth + 1));
    if (value.length > maxItems) trimmed.push(`…(${value.length - maxItems} more)`);
    return trimmed;
  }
...
const trimProjectData = (projectData) => {
  ...
  for (const cfg of configs) {
    const trimmed = deepTrim(projectData, cfg.maxItems, cfg.maxStrLen, cfg.maxDepth);
...
const generateWithFallback = async (prompt) => {
  for (const modelName of MODEL_FALLBACK_CHAIN) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(`[AI] Attempt ${attempt}/${MAX_RETRIES} using model: ${modelName}`);
        ...
        if (status === 404 || message.includes("not found") || message.includes("decommissioned")) {
        ...
        if (status === 413 || message.includes("Request too large")) {
          console.warn(`[AI] Prompt too large for ${modelName}. Trying next model...`);
          break;
        }
        ...
        if (status === 429) {
          ...
          console.warn(`[AI] Rate limit retries exhausted for ${modelName}. Trying next...`);
          break;
        }
    ...
    console.error(`[AI] All attempts failed for ${modelName}:`, lastError?.message);
  }
